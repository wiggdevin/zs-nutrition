...
test('Generate endpoint prevents duplicate jobs', () => {
  ...
  assert(routeCode.includes("status: { in: ['pending', 'running'] }"), 'Should check pending/running status');
...
test('Worker extracts job data correctly', () => {
  ...
  assert(workerCode.includes('const { intakeData, jobId } = job.data'), 'Should extract intakeData and jobId');
  assert(workerCode.includes('orchestrator.run(intakeData'), 'Should pass intakeData to orchestrator');
});

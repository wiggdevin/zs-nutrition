...
// Dev mode sign-up form that simulates the Clerk flow
// Creates a real user in the database via /api/dev-auth/signup
export function DevSignUpForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Simulate sending magic link (brief delay for UX)
    setTimeout(() => {
      setIsLoading(false);
      setStep('verify');
    }, 1000);
  ...
  const handleOAuthSignUp = async () => {
    ...
    try {
      // For OAuth simulation, generate a dev email
      const devEmail = `dev-google-${Date.now()}@gmail.com`;
      const response = await fetch('/api/dev-auth/signup', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      <div className="w-full rounded-xl border border-[#2a2a2a] bg-[#1a1a1a] p-6 shadow-2xl">
        <div className="space-y-4 text-center">
          ...
          <p className="text-sm text-[#a1a1aa]">
            We sent a verification link to <span className="text-[#fafafa]">{email}</span>
          </p>
          <p className="text-xs text-[#a1a1aa]/60 font-mono">
            [DEV MODE] Click below to simulate verification
          </p>
          {error && <p className="text-xs text-[#ef4444]">{error}</p>}
          <button
            onClick={handleVerify}
            disabled={isLoading}
            className="w-full rounded-lg bg-[#f97316] px-4 py-2.5 text-sm font-bold uppercase tracking-wide text-white transition-colors hover:bg-[#ea580c] disabled:opacity-50"
          >
            {isLoading ? (
  ...
  return (
    <div className="w-full rounded-xl border border-[#2a2a2a] bg-[#1a1a1a] p-6 shadow-2xl">
      <div className="space-y-1 mb-6">
        <h2 className="text-xl font-bold text-[#fafafa]">Create your account</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <input
            ...
            required
          />
          {error && step === 'email' && <p className="mt-1 text-xs text-[#ef4444]">{error}</p>}
        </div>
        <button
          type="submit"
          disabled={isLoading}
      ...
      <p className="mt-4 text-center text-sm text-[#a1a1aa]">
        ...
        <Link href="/sign-in" className="font-medium text-[#f97316] hover:text-[#ea580c]">
          Sign in
        </Link>
      </p>

'use client';

import { useState } from 'react';
import { ScrollReveal } from './ScrollReveal';

const testimonials = [
  {
    name: 'Alex K.',
    result: 'Lost 12 lbs in 8 weeks',
    quote:
      'The AI-generated meal plans actually fit my schedule. I stopped skipping meals and my energy levels are through the roof.',
    avatar: 'A',
  },
  {
    name: 'Sarah M.',
    result: 'Hit protein goals consistently',
    quote:
      'As a vegetarian, hitting my protein targets was always a struggle. Zero Sum found combinations I never would have thought of.',
    avatar: 'S',
  },
  {
    name: 'Mike D.',
    result: 'Gained 8 lbs lean mass',
    quote:
      "The training day vs rest day macro adjustments made a real difference. My bulk is clean and I'm not gaining unnecessary fat.",
    avatar: 'M',
  },
];

export function Testimonials() {
  const [hoveredIndex, setHoveredIndex] = useState<number | null>(null);

  return (
    <div className="grid grid-cols-1 gap-6 sm:grid-cols-3">
      {testimonials.map((t, i) => (
        <ScrollReveal key={t.name} delay={i * 120}>
          <div
            onMouseEnter={() => setHoveredIndex(i)}
            onMouseLeave={() => setHoveredIndex(null)}
            className={`rounded-2xl border border-border border-l-2 border-l-primary/40 bg-card p-6 h-full transition-all duration-500 ${
              hoveredIndex === i
                ? 'border-l-primary shadow-xl shadow-primary/5 -translate-y-1'
                : ''
            }`}
          >
            <div className="flex gap-1 mb-3">
              {[...Array(5)].map((_, j) => (
                <svg
                  key={j}
                  className={`h-4 w-4 transition-all duration-300 ${
                    hoveredIndex === i ? 'text-primary scale-110' : 'text-primary/70'
                  }`}
                  style={{ transitionDelay: `${j * 50}ms` }}
                  fill="currentColor"
                  viewBox="0 0 20 20"
                >
                  <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                </svg>
              ))}
            </div>
            <p className="text-sm leading-relaxed text-muted-foreground italic">
              &ldquo;{t.quote}&rdquo;
            </p>
            <div className="mt-4 flex items-center gap-3">
              <div className="flex h-9 w-9 items-center justify-center rounded-full bg-primary/10 text-sm font-bold text-primary ring-2 ring-primary/20">
                {t.avatar}
              </div>
              <div>
                <p className="text-sm font-semibold text-foreground">{t.name}</p>
                <p className="text-xs text-primary font-mono">{t.result}</p>
              </div>
            </div>
          </div>
        </ScrollReveal>
      ))}
    </div>
  );
}

...
export async function GET() {
  ...
  try {
    const input = {
      ...
      trainingDays: ['monday', 'wednesday', 'friday'] as Array<
    ...
    const cuisineCheck = {
      ...
      pass: result.cuisinePreferences.length === 1 && result.cuisinePreferences.includes('italian'),
    };

    const checks = {
      allergies: allergiesCheck,
      exclusions: exclusionsCheck,
      cuisinePreferences: cuisineCheck,
    };
  ...
  try {
    const input2 = {
      ...
      trainingDays: ['tuesday', 'thursday'] as Array<
        'monday' | 'tuesday' | 'wednesday' | 'thursday' | 'friday' | 'saturday' | 'sunday'
      >,
    ...
    const checks = {
      emptyAllergies: {
        expected: [],
        actual: result2.allergies,
        pass: result2.allergies.length === 0,
      },
      emptyExclusions: {
        expected: [],
        actual: result2.exclusions,
        pass: result2.exclusions.length === 0,
      },
    };

...
export default function TestFeature150Page() {
  ...
  const {
    ...
    error,
    refetch,
  } = trpc.plan.getJobStatus.useQuery(
    { jobId },
    {
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-[#1a1a1a] rounded-lg p-6">
          ...
          <ul className="space-y-2">
            <li
              className={`flex items-center ${jobStatus?.status ? 'text-green-400' : 'text-gray-400'}`}
            >
              <span className="mr-2">{jobStatus?.status ? '✅' : '○'}</span>
              Status field returns (pending/running/completed/failed)
            </li>
            <li
              className={`flex items-center ${jobStatus?.currentAgent !== undefined ? 'text-green-400' : 'text-gray-400'}`}
            >
              <span className="mr-2">{jobStatus?.currentAgent !== undefined ? '✅' : '○'}</span>
              currentAgent field present
            </li>
            <li
              className={`flex items-center ${jobStatus?.progress ? 'text-green-400' : 'text-gray-400'}`}
            >
              <span className="mr-2">{jobStatus?.progress ? '✅' : '○'}</span>
              progress field present
            </li>
            <li
              className={`flex items-center ${jobStatus?.planId ? 'text-green-400' : 'text-gray-400'}`}
            >

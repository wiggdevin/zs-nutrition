...
export const adaptiveNutritionRouter = router({
  ...
  analyzeWeightTrend: protectedProcedure.query(async ({ ctx }) => {
    ...
    if (recentEntries.length >= 2) {
      ...
      const recentTimeSpanDays = Math.max(
        1,
        Math.round((recentLast.logDate.getTime() - recentFirst.logDate.getTime()) / (1000 * 60 * 60 * 24))
      )
    ...
    const isOnTrack = Math.abs(weeklyRateLbs - (profile.goalType === 'cut' ? -goalRateLbsPerWeek : profile.goalType === 'bulk' ? goalRateLbsPerWeek : 0)) < 0.5
  ...
  applyCalorieAdjustment: protectedProcedure
    .input(
      z.object({
        newGoalKcal: z.number().min(1200, 'Calories must be at least 1200').max(5000, 'Calories must be 5000 or less'),
        confirmed: z.boolean(),
      })
    )
    .mutation(async ({ ctx, input }) => {
      ...
      if (trend.length >= 2) {
        const firstEntry = trend[0]
        const lastEntry = trend[trend.length - 1]
        const timeSpanDays = Math.round((lastEntry.logDate.getTime() - firstEntry.logDate.getTime()) / (1000 * 60 * 60 * 24))
      ...
      // Check for milestone
      const milestoneAchieved: string | null = null
      if (trend.length >= 2 && Math.abs(weightChangeLbs!) >= 5) {
        const milestone = Math.floor(Math.abs(weightChangeLbs!) / 5) * 5
        if (milestone >= 5 && milestone <= 25) {
          // Will be set during response
        }
      }

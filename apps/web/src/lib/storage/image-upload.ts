 ...
 */
export async function uploadFoodImage(base64Data: string, userId: string): Promise<string> {
  ...
  if (blobToken && !isDevMode) {
    try {
      ...
      if (!match) {
        logger.error(
          '[uploadFoodImage] Invalid data URL format, falling back to base64',
          new Error('Invalid data URL format')
        );
        return base64Data;
      }

...
export function generatePlanPdf(planData: PdfPlanData, metadata: PdfMetadata): Buffer {
  ...
  if (planData.weeklyTotals) {
    ...
    y -= 18
    page1Lines.push(textLine(60, y, `Avg Calories: ${planData.weeklyTotals.avgKcal} kcal | Protein: ${planData.weeklyTotals.avgProteinG}g | Carbs: ${planData.weeklyTotals.avgCarbsG}g | Fat: ${planData.weeklyTotals.avgFatG}g`))
    y -= 15
  }
  ...
  for (const day of planData.days) {
    ...
    page1Lines.push(boldTextLine(60, y, `${day.dayName}${trainingTag} - ${day.targetKcal} kcal`, 10))
    y -= 14
    for (const meal of day.meals) {
      if (y < 80) break
      page1Lines.push(textLine(80, y, `${meal.slot}: ${meal.name} (${meal.nutrition.kcal} kcal, P:${meal.nutrition.proteinG}g C:${meal.nutrition.carbsG}g F:${meal.nutrition.fatG}g)`, 8))
      y -= 12
    }
    y -= 6
  }
  ...
  for (const day of planData.days) {
    ...
    for (const meal of day.meals) {
      ...
      py -= 16
      pageLines.push(textLine(60, py, `Calories: ${meal.nutrition.kcal} | Protein: ${meal.nutrition.proteinG}g | Carbs: ${meal.nutrition.carbsG}g | Fat: ${meal.nutrition.fatG}g${meal.nutrition.fiberG ? ` | Fiber: ${meal.nutrition.fiberG}g` : ''}`, 9))
      py -= 14
  ...
  // Fonts
  addObj(fontObj, '<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica /Encoding /WinAnsiEncoding >>')
  addObj(boldFontObj, '<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica-Bold /Encoding /WinAnsiEncoding >>')
  ...
  for (let i = 0; i < pages.length; i++) {
    ...
    // Page object
    addObj(pageObjIds[i], `<< /Type /Page /Parent ${pagesObj} 0 R /MediaBox [0 0 612 792] /Contents ${contentObjIds[i]} 0 R /Resources << /Font << /F1 ${fontObj} 0 R /F2 ${boldFontObj} 0 R >> >> >>`)
  }

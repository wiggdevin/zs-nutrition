...
export default function QuickAddForm() {
  ...
  const quickAddMutation = trpc.meal.quickAdd.useMutation({
    onSuccess: (data) => {
      ...
      try {
        window.history.replaceState(
          { ...window.history.state, quickAddSubmitted: true },
          ''
        )
      } catch { /* ignore */ }
      // Reset form
    ...
    onError: (err) => {
      ...
      const isNetwork = msg.includes('fetch') || msg.includes('network') || msg.includes('Failed to fetch') || msg.includes('ECONNREFUSED') || msg.includes('NetworkError') || !navigator.onLine
      if (isNetwork) {
  ...
  return (
    <div className="mt-4 bg-[#1a1a1a] border border-[#333] rounded-xl p-5">
      ...
      {/* Success message */}
      {successMessage && (
        <div className="mt-3 px-4 py-2.5 bg-green-500/10 border border-green-500/30 rounded-lg text-green-400 text-sm flex items-center gap-2" data-testid="quick-add-success">
      ...
      {/* Error message */}
      {errorMessage && (
        <div className="mt-3 px-4 py-2.5 bg-red-500/10 border border-red-500/30 rounded-lg text-sm" data-testid="quick-add-error">

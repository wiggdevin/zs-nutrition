...
async function testFeature270Final() {
  ...
  cookies.cookies.forEach(c => {
    console.log(`    - ${c.name}: ${c.value.substring(0, 20)}... (expires: ${new Date(c.expires * 1000).toISOString()})`);
  });
  ...
  const finalCheck = await page2.evaluate(() => {
    ...
    const signInHeading = Array.from(document.querySelectorAll('h1, h2'))
      .find(h => h.textContent.toLowerCase().includes('sign in'));
    const bodyText = document.body.textContent.toLowerCase();
    return {
  ...
  console.log(`  4. Verify still signed in:              ${signedIn2.hasSignInLink ? '✗ FAIL' : '✓ PASS'} (no sign-in link)`);
  console.log(`  5. Verify dashboard loads with data:    ${canAccessProtected ? '✓ PASS' : '✗ FAIL'}`);
  console.log(`  6. Verify no re-authentication:         ${hasSignInForm ? '✗ FAIL' : '✓ PASS'} (no sign-in form)`);
...
testFeature270Final().then(result => {
  console.log('\n' + '='.repeat(60));

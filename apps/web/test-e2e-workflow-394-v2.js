...
async function testEndToEndWorkflow() {
  ...
  try {
    ...
    const mondayBtn = await page.locator('button[data-day="monday"], button:has-text("Mon")').count();
    if (mondayBtn > 0) {
    ...
    await page.click('button[data-testid="onboarding-complete-btn"], button:has-text("Complete Setup")');
    ...
    const macroRings = await page.locator('[data-testid^="macro-ring-"], svg[class*="ring"]').count();

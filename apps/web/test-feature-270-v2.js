...
async function testFeature270Correctly() {
  ...
  console.log('This test properly simulates browser close/reopen by saving and restoring cookies\n');
  ...
  console.log('  Cookies:', storageState.cookies.map(c => ({ name: c.name, expires: c.expires })));
  ...
  const dashboardCheck = await page2.evaluate(() => {
    ...
    return {
      ...
      hasDashboard: bodyText.includes('dashboard') || bodyText.includes('calories') || bodyText.includes('protein'),
  ...
  const sessionRecovered = !isSignIn && !dashboardCheck.hasSignInForm &&
...
testFeature270Correctly().then(result => {
  console.log('\n=== RESULT ===');

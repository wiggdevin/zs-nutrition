...
console.log(`  → Generated ${draft.days.length} days with ${draft.days[0].meals.length} meals per day`);
console.log('');
...
for (let i = 0; i < proteinsByDay.length - 1; i++) {
  ...
  const significantOverlap = overlap.filter(p =>
    !['mixed', 'dairy', 'eggs', 'whey', 'tofu', 'beans', 'chickpeas', 'lentils', 'soy'].includes(p)
  );

  if (significantOverlap.length > 0) {
    consecutiveProteinViolation = true;
    console.log(`  ❌ Days ${i + 1} and ${i + 2} share significant proteins: ${significantOverlap.join(', ')}`);
  }
}
  ...
  if (days.length > 1) {
    ...
    for (let i = 0; i < days.length - 1; i++) {
      ...
      if (dayDiff < 3) {
        ...
        console.log(`  ❌ Meal "${mealName}" appears on days ${days.join(', ')} (${dayDiff} days apart, need ≥3)`);
      }
    }
  }
}
...
assert(maxSingleCuisinePercentage < 70, `No single cuisine dominates >70% of meals (max: ${maxSingleCuisinePercentage.toFixed(1)}%)`);
console.log('');
...
assert(draft.varietyReport.proteinsUsed.length > 0, `varietyReport.proteinsUsed has ${draft.varietyReport.proteinsUsed.length} proteins`);
console.log(`  → proteinsUsed (${draft.varietyReport.proteinsUsed.length}): ${draft.varietyReport.proteinsUsed.join(', ')}`);

// Check cuisinesUsed
assert(Array.isArray(draft.varietyReport.cuisinesUsed), 'varietyReport.cuisinesUsed is an array');
assert(draft.varietyReport.cuisinesUsed.length > 0, `varietyReport.cuisinesUsed has ${draft.varietyReport.cuisinesUsed.length} cuisines`);
console.log(`  → cuisinesUsed (${draft.varietyReport.cuisinesUsed.length}): ${draft.varietyReport.cuisinesUsed.join(', ')}`);
...
if (apiKey && apiKey !== '' && !apiKey.includes('YOUR_KEY')) {
  ...
  if (claudeDraft && claudeDraft.days) {
    ...
    assert(Array.isArray(claudeDraft.varietyReport.proteinsUsed), 'Claude draft has proteinsUsed array');
    assert(Array.isArray(claudeDraft.varietyReport.cuisinesUsed), 'Claude draft has cuisinesUsed array');
    ...
    for (let i = 0; i < claudeProteinsByDay.length - 1; i++) {
      ...
      if (significantOverlap.length > 0) {
        ...
        console.log(`  ⚠️  Claude draft has consecutive protein overlap: ${significantOverlap.join(', ')}`);
      }
    }
      ...
      if (days.length > 1) {
        for (let i = 0; i < days.length - 1; i++) {
          ...
          if (dayDiff < 3) {
            ...
            console.log(`  ⚠️  Claude draft has meal repetition: "${mealName}" on days ${days.join(', ')}`);
          }
        }
      }
    }
